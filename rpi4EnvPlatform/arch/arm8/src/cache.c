...
void get_cache_line_a72_l2(cache_line *line, uint64_t set, uint64_t way) {
  volatile uint64_t value;
  BARRIER_DSB_ISB();
  
  // get info
  value = 0;
  value |= (0b11111111 & 0x10) << 24;
  value |= (0b1111 & way) << 18;
  value |= (0b1111111111 & set) << 6;
  ...
  line->r0 = value;
  line->tag = (((0x7FFFFFFC&value) >> 2) << 15);  
...
void save_cache_l2_state(cache_l2_state cache) {
  BARRIER_DSB_ISB();

  for (int set=0; set<SETS_L2; set++) {
    for (int way=0; way<WAYS_L2; way++) {
         get_cache_line_a72_l2(&(cache[set][way]), set, way);
    }
  }
...
void print_cache_l2_valid(cache_l2_state c) {
  printf("----\n");
  printf("print_cache_valid\n");
  printf("L2 output\n");
  printf("----\n");
  for (uint64_t set=0; set<SETS_L2; set++) {
